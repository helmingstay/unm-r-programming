...
namespace Rcpp{
    ...
    namespace RcppArmadillo{
        ...
        template <typename INDEX>
        void WalkerProbSampleReplace(INDEX &index, int nOrig, int size, arma::vec &prob);
        ...
        T sample(const T &x, const int size, const bool replace, NumericVector prob_ = NumericVector(0) ) {
            ...
            if (probsize == 0) { // No probabilities given
                ...
                // Copy isn't necessary, but preserves R's dont-modify-args semantics
                ...
                if (replace) {
                    ...
                    if (walker_test < 200) { 
                        ...
                        WalkerProbSampleReplace<IntegerVector>(index, nOrig, size, prob);
                    }
                } else {
        ...
        void WalkerProbSampleReplace(INDEX &index, int nOrig, int size, arma::vec &prob){
            ...
            IntegerVector HL(nOrig); // index table
            IntegerVector alias_tab(nOrig); // index table
            IntegerVector::iterator H, H0 = HL.begin();
            IntegerVector::iterator L, L0 = HL.end()-1;
            prob *= nOrig; // scale probability table
            // fill HL from beginning (small prob) and end (large prob) with indices
            for (ii = 0; ii < nOrig; ii++) {
                if( prob[ii] < 1.) {
                    *H = ii;
                    H++;
            ...
            for (ii = 0; ii < size; ii++) {
                rU = unif_rand() * nOrig;
                kk = (int) rU;
                index[ii] = (rU < prob[kk]) ? kk+1 : alias_tab[kk]+1;
            }
        }
