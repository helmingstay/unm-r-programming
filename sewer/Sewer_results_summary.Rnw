...
=======
paste('Pseudo $R^2$ = ', signif(pr2, 2))  # 0.093
>>>>>>> 0eb2183c941ba7f57a07e8a8ce8c5d878570560d
@
...
not.on.holiday <- sewer.fail.grease$N[-
  which(sewer.fail.grease$week == 51 | # xmas. no blockages in xmas 2012
          sewer.fail.grease$week == 48 & sewer.fail.grease$year == 2009 | # thanksgiving 09
          sewer.fail.grease$week == 47 & sewer.fail.grease$year >= 2010  ) # thanksgivings
  ]

t.test(holiday.blocks, not.on.holiday) # nonsig
...
# do bootstrapping
bs.hols <- bootstrap(holiday.blocks)

# combine for plotting; sort for fitting confidence intervals
bs.hols.df <- data.frame(sort(bs.hols))
names(bs.hols.df) <- 'blocks'

# plot!
p <- ggplot(bs.hols.df, aes(x=blocks))
...
p <- p + geom_vline(aes(xintercept=mean(not.on.holiday)), colour="#BB0000", linetype="dashed")
# add 95% CIs of holiday weeks in green
p <- p + geom_vline(aes(xintercept=bs.hols.df$blocks[0.025*nrow(bs.hols.df)]), colour="#00AA00", linetype="longdash")
p <- p + geom_vline(aes(xintercept=bs.hols.df$blocks[0.975*nrow(bs.hols.df)]), colour="#00AA00", linetype="longdash")
...
Even without accounting for temperature, the weeks of Thanksgiving and Christmas do not have an exceptionally high number of blockages.
