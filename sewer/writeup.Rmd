...
```{r block.airtemp, echo=FALSE, fig.pos='h', fig.width=4.5, fig.height=3.5, fig.cap=sprintf('Mean weekly air temperature predicts blockages caused by grease (A: %d total blocks), and is a poor predictor of blockages due to other causes (B: %d total blocks). %d weeks total, pseudo-R2 (D) %s and %s for grease and non-grease blockage frequency, respectively. See Table ?? for model details.', block.counts$grease, block.counts$not.grease,  block.airtemp.list$nweeks, block.airtemp.list$dev.grease, block.airtemp.list$dev.nogrease ), message=FALSE}
