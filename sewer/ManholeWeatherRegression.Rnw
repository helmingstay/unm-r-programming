...
p <- p + geom_point(aes(x=MeanTempC, y=SewTempC, colour=Interceptor), shape=21)
p <- p + geom_smooth(aes(x=MeanTempC, y=SewTempC, colour=Interceptor), method='lm')
p <- p + theme_bw()
print(p)
@
...
## show summary of best linear and mixed mode;
summary(.lin.best[[1]])
summary(.mix.best[[1]])
@
